...
;(function($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        init: function() {
            ...
            var    $child = $("<div>", { "class": this.settings.place }).appendTo($template);
                ...
                $.each(networks, function(k, v) {
                    if (value === k) {
                        var $icon = $("<i>", { "class": "margin-top-5 fa fa-" + value }),
                            ...
                            $component = $("<a>", { title: base.settings.title, "class": v.className + " pop-upper"}).attr("href", _href).attr("title", base.settings.text + value).attr("target", "_blank").append($icon);
    ...
    var places = ["top-left", "top-right"],
        networks = {
            ...
            "wordpress": {
                className: "wordpress",
                url: "http://wordpress.com/press-this.php?u={url}&t={title}&s={description}"
            },
            "vk": {
                className: "vk",
    ...
    function setShareCount(network, url, $component) {
        switch(network) {
            ...
            case "twitter":
                ...
                break;
            case "wordpress":
                $.get('http://wordpress.com/press-this.php?u='+ url +'&callback=?', function(data) {
                    appendButtons(data.count, $component);
                },'jsonp');
                break;
            case "vk":
